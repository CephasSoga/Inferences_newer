...
class TrendEngine:
    ...
    def __init__(self, keywords: list[str] = DEFAULT_KEYWORDS, category_id: int = DEFAULT_CAT_ID, delta: int = 2, hl: str = 'en-US', geo: str = None, timeout: int = None, engine_timeout: tuple[int, int] = None,  proxies: str = None, retries: int = 3, backoff_factor: int = 1) -> None:
        ...
        self.logger = Logger("Inference-TrendEngine")
    ...
    def _related_queries_payload(self) -> dict:
        ...
        try:
            ...
            self.logger.log("error", f"ResponseError while building payload [_related_queries_payload]: {response_error}")
            self.logger.log("info", "Recovering strategy: Rotationg alternative keywords")

            while not success:

                try:

                    key = get_next_key(KEYWORDS_MAP, iteration)
                    if not key:
                        self.logger.log("info", "No more alternative keywords available. Exiting...")
                        break

                    self.keywords = KEYWORDS_MAP[key]
                    ...
                    self.logger.log("error", f"Failed to recover from ResponseError [_related_queries_payload]: {response_error}")

                    success = False
                    iteration += 1

                    if iteration > max_iterations:
                        self.logger.log("error", f"Max retries reached while building payload [_related_queries_payload]: {response_error}")
                        break

                    sleep_time = sleep_time * (iteration**backoff)
                    self.logger.log("info", f"Retrying in {sleep_time} seconds...")
                    time.sleep(sleep_time)

                except IndexError as index_error:
                    self.logger.log("error", f"Failed to recover from IndexError [_related_queries_payload]: {index_error}")
                    return

        except Exception as e:
            self.logger.log("error", f"Uncaught Error while building payload [_related_queries_payload]: {e}")
            return 
