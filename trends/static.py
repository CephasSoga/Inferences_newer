...
logger = Logger("Inference-Static")
...
def reload_kwds(kwds: list[str] = DEFAULT_KEYWORDS) -> list[str]:
    """
    ...
    if len(kwds) < 2:
        raise ValueError("Need to return at least two keywords")
...
def get_next_key(map: dict[str, Any], iteration: int = 0) -> str | None:
    try:
        keys = list(map.keys())
